...
export default function StudentMessages() {
  ...
  // Mobile: track whether to show chat panel or list panel
  ...
  return (
    <DashboardLayout title="Messages">
      <div className="flex gap-0 h-[calc(100vh-8rem)] sm:h-[calc(100vh-9rem)] -mx-3 sm:-mx-5 lg:-mx-8 my-0 overflow-hidden rounded-xl sm:rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-card backdrop-blur-md">
        ...
        `}>
          <div className="p-4 sm:p-5 border-b border-[var(--border)] bg-[var(--bg-secondary)]/30">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative flex-shrink-0">
                      {other?.avatar ? (
                        <img src={other.avatar} className="w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover flex-shrink-0 border border-[var(--border)]" alt="" />
                      ) : (
                        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-brand-400 to-violet-500 flex items-center justify-center text-white font-bold flex-shrink-0 text-sm">
                          {other?.name?.charAt(0)}
                        </div>
                      )}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="inline-block max-w-[160px] truncate text-[9px] font-black tracking-wide uppercase text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-950/40 px-2 py-0.5 rounded-md mt-1 border border-brand-500/10">
                        {chat.project?.title}
                      </span>
                      {isTyping ? (
